//  Class ini diambil dari internet untuk mengukur panjang dan luas dengan ARKit
//  RKPathEntity.swift
//  RKPathMaker+Example
//
...
//            let middlePoint : simd_float3 = simd_float3((newPosition.x + lastPosition.x)/2, (newPosition.y + lastPosition.y)/2, (newPosition.z + lastPosition.z)/2)
//            if length > 0.0{
//                let textSegment = makeTextEntity(length: length)
//                textSegment.orientation = simd_quatf(angle: .pi,    /* 45 Degrees */
//                                                     axis: [1,0,0])
//                textSegment.position = middlePoint
...
//            }
    ...
    func makePathSegment(length: Float, width: Float = 0.005) -> Entity {
        ...
        let generatedRectangle = ModelEntity(mesh: rectangleMesh,
                                             materials: pathMaterials ?? [UnlitMaterial.init(color: .white)])
...
//        let generatedText = ModelEntity(mesh: textMesh, materials: [UnlitMaterial.init(color: .white)])
//        
